...
void writeLinkCell(FILE *fp, LinkCell* boxes) {
	...
	int lboxes;
	lboxes = boxes->nLocalBoxes;
	fwrite(&lboxes, sizeof(int), 1, fp);

	int hboxes;
	hboxes = boxes->nHaloBoxes;
	fwrite(&hboxes, sizeof(int), 1, fp);

	int tboxes;
	tboxes = boxes->nTotalBoxes;
	fwrite(&tboxes, sizeof(int), 1, fp);
	...
	fprintf(screenOut, "Total number of Boxes in writelinkcell %d\n", boxes->nTotalBoxes);
...
LinkCell* readLinkCell(FILE *fp) {
	...
	int tboxes;
	fread(&tboxes, sizeof(int), 1, fp);
	ll->nTotalBoxes = tboxes;
	...
	fprintf(screenOut, "Total number of Boxes in readlinkcell %d\n", ll->nTotalBoxes);
	ll->nAtoms = comdMalloc(ll->nTotalBoxes*sizeof(int));
	fread(ll->nAtoms, sizeof(int), tboxes, fp);
