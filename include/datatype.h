...
class Datatype
{
  ...
  public: 
    ...
    const MPI_Datatype &getMpiDatatype() const {return mpiDatatype;}
    const size_t getExtent() const {return extent;}
    const bool isAssociative() const {return associative;}
    const bool isWeakAssociative() const {return weakAssociative;}
    const bool isCommutative() const {return commutative;}
}; 
