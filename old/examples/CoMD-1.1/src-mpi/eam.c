...
void writeEamPot(FILE *fp, BasePotential* p, LinkCell* boxes) {
	EamPotential* pot = (EamPotential*) p;

	real_t c = pot->cutoff;
	real_t mp = pot->mass;
	real_t l = pot->lat;
	fwrite(&c, sizeof(real_t), 1, fp);
	fwrite(&mp, sizeof(real_t), 1, fp);
	fwrite(&l, sizeof(real_t), 1, fp);

	fwrite(pot->latticeType, sizeof(char), 8, fp);
	fwrite(pot->name, sizeof(char), 3, fp);

	int an = pot->atomicNo;
	fwrite(&an, sizeof(int), 1, fp);


	int phisize = pot->phi->n;
	fwrite(&phisize, sizeof(int), 1, fp);

	real_t xo = pot->phi->x0;
	fwrite(&xo, sizeof(real_t), 1, fp);
	real_t invdx = pot->phi->invDx;
	fwrite(&invdx, sizeof(real_t), 1, fp);
...
BasePotential* readEamPot(FILE *fp, LinkCell* boxes) {
	EamPotential* pot = comdMalloc(sizeof(EamPotential));
	pot->force = eamForce;
	pot->print = eamPrint;
	pot->destroy = eamDestroy;
	...
	fread(&pot->phi->x0, sizeof(real_t), 1, fp);
	fread(&pot->phi->invDx, sizeof(real_t), 1, fp);
	fread(pot->phi->values, sizeof(real_t), phisize + 3, fp);

	int rhosize;
	fread(&rhosize, sizeof(int), 1, fp);
	pot->rho->n = rhosize;

	fread(&pot->rho->x0, sizeof(real_t), 1, fp);
	fread(&pot->rho->invDx, sizeof(real_t), 1, fp);
	fread(pot->rho->values, sizeof(real_t), rhosize + 3, fp);

	int fsize;
	fread(&fsize, sizeof(int), 1, fp);
	pot->f->n = fsize;

	fread(&pot->f->x0, sizeof(real_t), 1, fp);
	fread(&pot->f->invDx, sizeof(real_t), 1, fp);
	fread(pot->f->values, sizeof(real_t), fsize + 3, fp);
